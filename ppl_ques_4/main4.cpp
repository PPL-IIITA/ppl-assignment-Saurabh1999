...
void Allocate(vector<boy> &Boy,vector<girl> &Girl,vector<couple>&Couple,bool new_allocation){
    ...
    if(new_allocation == false)
    	...
    	file.open("new_allocation_after_breakup.txt",ios_base::app);
...
void break_up(vector<couple> &Couple,vector<boy> &Boy,vector<girl> &Girl,int l){
	ofstream file;
	int i,j,k,count = 0;
	file.open("break_up.txt");
	file << "----------------------------performing breakup of 50 boys----------------------------------------" <<endl;
...
int main(){
    ...
    Allocate(Boy,Girl,Couple,false);   /*function to make couples before breakup */
    break_up(Couple,Boy,Girl,50); //performs breakup
   	Allocate(Boy,Girl,Couple,true); /*function to make couples after breakup */
